    ...
    mounted:function () {
        ...
        axios.get(json_url).then(response => {
            ...
            for(var num=0;num<this.pfeatures.length;num++){
                this.pfeatures_boolean_list[this.pfeatures[num].voR_id]={'boolean':false,'feature':this.pfeatures[num].feature,'type':this.pfeatures[num].type};
            }
            for(var num=0;num<this.ifeatures.length;num++){
                this.ifeatures_boolean_list[this.ifeatures[num].imR_id]={'boolean':false,'feature':this.ifeatures[num].feature,'type':this.ifeatures[num].type};
            }
            for(var num=0;num<this.vfeatures.length;num++){
                this.vfeatures_boolean_list[this.vfeatures[num].viR_id]={'boolean':false,'feature':this.vfeatures[num].feature,'type':this.vfeatures[num].type};
            }
            /* ルールごとにチェックボックスの論理値を初期状態としてfalseにする } */
        })
        /* jsonデータを読み出し、上記data内の変数に格納 } */
        ...
        click_post_button:function(){
            ...
            if(this.form_post_flag){
                axios.post(post_url,{
                    ...
                    "word_accurancy":{'wordAccurancy_id':this.word_accurancy_checkbox_id,'boolean':this.word_accurancy_checkbox_check,'order':this.word_accurancy_list[this.word_accurancy_checkbox_id].order,'percent':Number(modified_percent),'teacher_behavior':this.teacher_behavior},
